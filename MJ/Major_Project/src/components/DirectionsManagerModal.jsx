...
export default function DirectionsManagerModal({ isOpen, onClose, rooms, onSave }) {
          ...
          <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
            <div className="grid grid-cols-1 gap-4">
              {filteredRooms.map((room) => (
                <div key={room.id} className="group flex flex-col md:flex-row gap-4 p-6 bg-black/[0.02] dark:bg-white/[0.01] border border-black/5 dark:border-white/5 rounded-2xl hover:border-blue-500/30 transition-all">
                  <div className="w-full md:w-64 shrink-0">
                    <div className="flex items-center gap-3 mb-2">
                      <div className={`w-2 h-2 rounded-full ${room.type === 'lab' ? 'bg-emerald-500' : room.type === 'classroom' ? 'bg-blue-500' : 'bg-amber-500'}`} />
                      <h4 className="text-sm font-orbitron font-black uppercase tracking-widest text-black dark:text-white truncate">
                        {room.name}
                      </h4>
                    </div>
                    <div className="flex flex-col gap-1">
                      <span className="text-[9px] font-mono text-black/30 dark:text-white/20 uppercase tracking-tighter">ID: {room.id}</span>
                      <span className="text-[9px] font-orbitron font-bold text-blue-500/50 uppercase tracking-widest">{room.type}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 relative">
                    <textarea 
                      ...
                      onChange={(e) => handleDirectionChange(room.id, e.target.value)}
                    ...
                    />
                    {(!editedDirections[room.id] || editedDirections[room.id] === 'TBD') && (
                      <div className="absolute top-4 right-4 flex items-center gap-2 text-amber-500/50">
                        <AlertCircle className="w-3.5 h-3.5" />
                        <span className="text-[8px] font-orbitron font-black uppercase tracking-widest">Missing Data</span>
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="p-8 border-t border-black/5 dark:border-white/5 bg-black/[0.02] dark:bg-white/[0.02] flex items-center justify-between">
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-orbitron font-black text-black/40 dark:text-white/20 uppercase tracking-widest">
                  {rooms.filter(r => !r.directions || r.directions === 'TBD').length} Missing
                </span>
              </div>
            </div>
